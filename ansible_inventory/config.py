...
class IniSettings(BaseSettings):
    ...
    Example:
      settings = IniSettings(section='digital_ocean',
                             filename='digital_ocean.ini')
...
class EnvironmentSettings(BaseSettings):
    ...
    Example:
      settings = EnvironmentSettings(prefix='DO')
      settings.get('api_key')  # reads: DO_API_KEY
    ...
    def get(self, option):
        val = getenv(
            '{0}_{1}'.format(self.prefix.upper(), option.upper()),
            None
        )
